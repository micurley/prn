  ...
  module LunrJsSearch
    class Indexer < Jekyll::Generator
      def initialize(config = {})
        ...
        lunr_config = {
          'excludes' => ['index.html', 'categories.html', 'archive.html', 'pages.html', 'tags.html'],
        ...
        raise "Could not find #{@lunr_path} -> #{gem_lunr}" if !File.exist?(@lunr_path)
      ...
      def generate(site)
        ...
        Jekyll.logger.debug "Lunr:", "Title #{entry.title}"
        Jekyll.logger.debug "Lunr:", entry.body
        ...
        Jekyll.logger.debug "Lunr:", "File #{@filepath}"
  ...
  module LunrJsSearch
    class PageRenderer
      ...
      def render(item)
        ...
        Nokogiri::HTML(layoutless.output).text.gsub(/[^\s]{99,}/, "").gsub(/\W+/, ' ')
      end
    end
  end
end
